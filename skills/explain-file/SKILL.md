...
**Tier 1 — IDE-injected harness tags.** Check whether the message that invoked this skill carries `<ide_opened_file>` (or an equivalent editor-event tag naming the active file) — including one riding inside a `<system-reminder>` block. When present, this is the fastest path: no tool call, no file Read, use it straight away.

  1. **Only the tag attached to this invocation's message counts.** Do not mine earlier turns in the conversation for one — a tag from a prior message no longer reflects what's open now, and using it would be indistinguishable from guessing.
  2. **Absence here is not "no file is open."** These tags are one-shot *change* events: the harness attaches one only to the message where the file changed, and sends nothing on every later message even though that file may still be open. So if this message carries no such tag, that just means nothing changed on *this* message — fall through to Tier 2, don't treat it as a dead end.
  3. If the tag is present, resolve the path it names (absolute, or relative to the current working directory / project root) and skip Tiers 2–3 — this is the target. Treat it exactly like an explicit argument from here on.
...
**Tier 4 — ask.** If Tiers 0–3 found nothing:
...
- Once resolved via path, read the file with the Read tool before doing anything else (skip this if Tier 3 step 3 already supplied the text directly).
...
If the target was resolved from text content only (no on-disk path, per Tier 3 step 3), use the filename/location the user gave you when asked, with the extension replaced by `.$.md` the same way.
